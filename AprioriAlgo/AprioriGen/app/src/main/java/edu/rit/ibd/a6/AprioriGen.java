...
public class AprioriGen {
	...
	public static void main(String[] args) throws Exception {
		...
		// TODO Your code here!
		List<Document> lkminusone = lKMinusOne.find().into(new ArrayList<Document>());
		

		Document first = lkminusone.get(0);
		Document items  = (Document) first.get("items");
		int k = items.size();
		//if(k==1){
			for(Document docp : lkminusone){
				Document dp = (Document) docp.get("items");
				int p = dp.getInteger("pos_0");
				for(Document docq : lkminusone){
					Document dq = (Document) docq.get("items");
					int q = dq.getInteger("pos_0");
					if(p<q){
